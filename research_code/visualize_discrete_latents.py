...
def main(
    ...
    for i in range(num_embeddings):
        ...
        latent_idcs = torch.ones(1,16*16, dtype=torch.long) * i
    ...
    for i, recon in enumerate(reconstructions):
        plt.figure()
        plt.imshow(einops.rearrange(recon, 'c h w -> h w c'))
        plt.savefig(os.path.join(save_dir, str(i) + '.png'))

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--vqvae_path', type=str, default='/home/lieberummaas/datadisk/minerl/run_logs/VQVAE/MineRLTreechopVectorObf-v0/lightning_logs/version_0/checkpoints/last.ckpt')
