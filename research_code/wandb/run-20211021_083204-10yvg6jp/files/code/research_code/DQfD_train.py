        ...
        with torch.no_grad():
            ...
            while not done:    
                ...
                if q_net.dynamics_model is not None:
                    ...
                    hidden_states_seq, _ = q_net.dynamics_model.gru(gru_input, torch.from_numpy(predictive_state_list[-1]).to(q_net.device)[None,None])
                ...
                if steps >= max_episode_len or total_env_steps == max_env_steps:
                    print('Ending episode prematurely!')
                    break
