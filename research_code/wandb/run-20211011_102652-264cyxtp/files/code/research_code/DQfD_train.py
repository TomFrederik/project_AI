...
def main(
    ...
    while total_env_steps < max_env_steps:
        ...
        with torch.no_grad():
            ...
            # compute q values
            if q_net.dynamics_model is not None:
                q_values = q_net(obs_pov, obs_vec, torch.from_numpy(predictive_state_list[-1])[None]).to(q_net.device)            
            else:
            ...
            while not done:    
                ...
                if q_net.dynamics_model is not None:
                    ...
                    # record td_error
                    td_error_list.append(np.abs(rew + discount_factor * q_net(obs_pov, obs_vec, torch.from_numpy(predictive_state_list[-1])[None].to(q_net.device), target=True)[0].squeeze()[torch.argmax(q_values)].cpu().item() - highest_q))
