    ...
    while total_env_steps < max_env_steps:
        ...
        with torch.no_grad():
            ...
            if q_net.dynamics_model is not None:
                ...
                predictive_state_list.append(hidden_states_seq[0])
                q_values = q_net(obs_pov, obs_vec, predictive_state_list[-1])            
            else:
        ...
        dataset.add_episode(obs_list, action_list, np.array(rew_list), td_error_list, np.zeros(len(obs_list)), memory_id='agent')
