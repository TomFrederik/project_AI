...
def main(
    ...
    while total_env_steps < max_env_steps:
        ...
        if q_net.dynamics_model is not None:
            predictive_state_list.append(np.zeros(1, q_net.dynamics_model.hparams.gru_kwargs['hidden_size']))
        ...
        with torch.no_grad():
            ...
            # compute q values
            if q_net.dynamics_model is not None:
                q_values = q_net(obs_pov, obs_vec, torch.from_numpy(predictive_state_list[-1]))            
            else:
            ...
            while not done:    
                ...
                # compute q values
                if q_net.dynamics_model is not None:
                    sample, *_ = q_net.dynamics_model.visual_model.encode_only(pov_obs) 
                    gru_input = torch.cat([sample, vec_obs, action_centroid], dim=1)[None]
                    hidden_states_seq, _ = dynamics_model.gru(gru_input)
                    predictive_state_list.append(hidden_states_seq[0].detach().cpu().numpy())
            
                    # record td_error
                    td_error_list.append(np.abs(rew + discount_factor * q_net(obs_pov, obs_vec, torch.from_numpy(predictive_state_list[-1]), target=True)[0].squeeze()[torch.argmax(q_values)].cpu().item() - highest_q))
        ...
        print('\nAdding episode to memory...') # TODO pass real predictive state
        dataset.add_episode(obs_list, action_list, np.array(rew_list), td_error_list, predictive_state_list, memory_id='agent')
        ...
        for i, batch in tqdm(enumerate(dataloader)):
            ...
            predictive_state = predictive_state.to(device)
            next_predictive_state = next_predictive_state.to(device)
            n_step_predictive_state = n_step_predictive_state.to(device)
