...
def main(
    ...
    # load centroids
    centroids_path = os.path.join(centroids_path, env_name + f'_{num_centroids}_centroids.npy') #TODO make sure that it uses the same centroids as in pretraining
    ...
    while total_env_steps < max_env_steps:
        ...
        with torch.no_grad():
            ...
            while not done:    
                ...
                # compute next predictive state h_t(a_t-1, z_t-1, h_t-1)
                if q_net.dynamics_model is not None:
                ...
                # compute q-values
                if q_net.dynamics_model is not None:
                    q_values = q_net(obs_pov, obs_vec, hidden_states_seq[0])[0].squeeze()
        ...
        for i, batch in tqdm(enumerate(dataloader)):
            ...
            wandb.log({
                ...
                'Training/percentage_expert_actions': expert_mask.detach().float().mean()
            })
        ...
        print('\nUpdating beta...')
        beta = min(beta + (1-beta_0)/max_env_steps*steps, 1)
