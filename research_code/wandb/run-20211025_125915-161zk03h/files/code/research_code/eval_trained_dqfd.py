...
def main(
    ...
    for i in range(num_episodes):
        ...
        if q_net.dynamics_model is not None:
            predictive_state = torch.from_numpy(np.zeros((1, q_net.dynamics_model.hparams.gru_kwargs['hidden_size'])).astype(np.float32)).to(q_net.device)
        ...
        while not done:
            ...
            if q_net.dynamics_model is not None:
                ...
                hidden_states_seq, _ = q_net.dynamics_model.gru(gru_input, predictive_state[None])
            ...
            if steps >= max_episode_len and (not done):
                ...
                break
        print(f'Finished episode {i+1} after {steps} steps.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_episode_len', type=int, default=7000)
    parser.add_argument('--model_path', type=str, required=True)
    parser.add_argument('--epsilon', type=float, default=1, help='Epsilon for epsilon-greedy behaviour')
