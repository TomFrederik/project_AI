...
class PredictionCallback(pl.Callback):
    ...
    def predict_sequence(self, trainer, pl_module: dynamics_models.MDN_RNN, epoch):
        ...
        if pl_module.hparams.visual_model_cls == 'vqvae':
            ...
            images = torch.stack([self.sequence[0][0,1:], one_step_pov_reconstruction, n_step_pov_reconstruction], dim=1).reshape(((self.seq_len -1) * 2, 3, 64, 64))

            # log images to tensorboard
            pl_module.logger.experiment.log({'Predictions (truth | one-step | n-step)': wandb.Image(make_grid(images, nrow=2))})
